...
generateFonts({
  ...
  fs.readFile(`${out_path}/vertical-brush.html`, 'utf8', function (err,data) {
    if (err) {
      return console.log(err);
    }
    const result = replace_problematic_css(data);
    fs.writeFile(`${out_path}/vertical-brush.html`, result, 'utf8', function (err) {
       if (err) return console.log(err);
    });
  });

  fs.readFile(`${out_path}/vertical-brush.css`, 'utf8', function (err,data) {
    if (err) {
      return console.log(err);
    }
    const result = replace_problematic_css(data);
    fs.writeFile(`${out_path}/vertical-brush.css`, result, 'utf8', function (err) {
       if (err) return console.log(err);
    });
  });
